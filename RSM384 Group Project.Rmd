...
nba.combine <- nba.combine %>% select (Player, YEAR, HGT, WGT, BAR, STNDRCH, PAN, LPVERT, SPRINT, LANE)
...
**Creating a Time Series measure of the Physical Attributes of the Top 10 Defenders each year (2003-2023):**

```{r}
top.10.defrtg.per.year <- nba.9624.merged %>%
...
top.10.defrtg.per.year %>% 
  select(PLAYER, YEAR, DEFRTG, HGT, WGT, BAR) %>% 
  arrange(desc(YEAR)) %>% 
  head(10)
```
...
```{r}
ggplot(top.10.defrtg.per.year, aes(x = YEAR, y = HGT, color = DEFRTG)) + 
...
```{r}
ggplot(top.10.defrtg.per.year, aes(x = YEAR, y = BAR, color = DEFRTG)) + 
  geom_smooth(method = "lm", se = FALSE, color = "blue", linetype = "dashed") + 
...
(Note: Hand Measurements only started in 2010)

```{r}
ggplot(top.10.defrtg.per.year, aes(x = YEAR, y = PAN, color = DEFRTG)) + 
  geom_smooth(method = "lm", se = FALSE, color = "blue", linetype = "dashed") + 
...
```{r}
ggplot(top.10.defrtg.per.year, aes(x = YEAR, y = LPVERT, color = DEFRTG)) + 
  geom_smooth(method = "lm", se = FALSE, color = "blue", linetype = "dashed") + 
